 ...
 * Date: 09/19/2014
 * 
 * Summary: Represents unit for particular quantity type, handles the calculations
 * conversions, parsing and other stuff.
 * *****************************************************************************/
...
namespace QuantityX
{
    ...
    public class Unit<T> : IUnit where T : unmanaged, IQuantity<T>
    {
        ...
        {
            return Parse(str, NumberStyles.Any, QuantityX.Culture, defaultUnit);
        }

        public static bool TryParse(string str, out T quantity, Unit<T> defaultUnit = null)
        {
            return TryParse(str, NumberStyles.Any, QuantityX.Culture, out quantity, defaultUnit);
        }
        ...
        static IUnit GetUnitFromSubstring(string str, out int unitEndIndex, bool byLetter = false)
        {
            ...
            while(length > 0)
            {
                ...
                var unit = QuantityX.GetUnitByName<T>(substr);
